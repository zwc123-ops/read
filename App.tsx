...
import { BookOpen, Library, Star, Brain, Plus, ChevronLeft, Layout } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Add the missing onBookUpload callback
  ...
  const handleWordSelect = useCallback(async (word: string) => {
    ...
    } catch (error: any) { alert(`分析失败: ${error.message}`); }
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-white overflow-hidden safe-bottom">
      {/* 侧边/底部导航 */}
      <nav className="fixed bottom-0 left-0 w-full h-16 md:relative md:w-20 md:h-screen bg-white/80 backdrop-blur-xl md:border-r border-gray-100 flex md:flex-col items-center justify-around md:justify-start py-0 md:py-8 z-[50] pb-safe">
        ...
        <div className="flex-1 flex md:flex-col items-center justify-around md:justify-center gap-2 md:gap-10 w-full">
          <button onClick={() => setView('home')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'home' ? 'text-indigo-600 md:bg-indigo-50' : 'text-gray-300'}`}>
            <Library className="w-6 h-6" />
            <span className="text-[8px] font-bold md:hidden">书架</span>
          </button>
          <button onClick={() => setView('collection')} className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${view === 'collection' ? 'text-indigo-600 md:bg-indigo-50' : 'text-gray-300'}`}>
            <Star className={`w-6 h-6 ${view === 'collection' ? 'fill-indigo-600' : ''}`} />
            <span className="text-[8px] font-bold md:hidden">收藏</span>
          </button>
          <button onClick={() => setIsSettingsOpen(true)} className="flex flex-col items-center gap-1 p-2 text-gray-300">
            <Brain className="w-6 h-6" />
            <span className="text-[8px] font-bold md:hidden">设置</span>
          </button>
        </div>
      </nav>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-8 md:px-12 md:py-16 bg-gray-50/30">
            <header className="max-w-6xl mx-auto flex justify-between items-end mb-10">
              ...
              <button onClick={() => setView('upload')} className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-2xl font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">
                <Plus className="w-5 h-5" /> <span className="hidden sm:inline">新书</span>
              </button>
            </header>

            <div className="max-w-6xl mx-auto grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8">
              {books.map(book => (
                <div key={book.id} onClick={() => { setCurrentBookId(book.id); setView('reader'); }} className="group cursor-pointer flex flex-col">
                  <div className="aspect-[3/4] mb-4 bg-white rounded-[2rem] border border-gray-100 flex items-center justify-center relative overflow-hidden shadow-sm group-hover:shadow-2xl group-hover:shadow-indigo-500/10 transition-all duration-500 group-hover:-translate-y-2">
                    {book.originalImage ? (
                       <img src={book.originalImage} className="w-full h-full object-cover opacity-80" alt="" />
                    ) : (
                  ...
                  <p className="text-[10px] text-gray-400 mt-1 uppercase font-black tracking-widest px-1">{progress[book.id] || 0}% 已读</p>
                </div>
              ))}
              {books.length === 0 && (
                <div onClick={() => setView('upload')} className="col-span-full py-20 border-2 border-dashed border-gray-200 rounded-[3rem] flex flex-col items-center justify-center text-gray-400 hover:border-indigo-300 hover:text-indigo-400 transition-all cursor-pointer">
                  <Plus className="w-12 h-12 mb-4 opacity-20" />
                  <p className="font-bold">还没有书籍，点击上传一本吧</p>
                </div>
              )}
            </div>
          </div>
